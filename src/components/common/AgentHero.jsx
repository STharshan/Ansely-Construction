import { ArrowRight, BriefcaseBusiness, Mail, MapPin, Pen, Phone } from "lucide-react";
  ...
  return (
    <section className="bg-[#f7f7f4] px-5 py-8 sm:px-8 sm:py-12 lg:px-14 lg:py-16">
      <div className="mx-auto grid max-w-[1520px] gap-14 lg:grid-cols-[610px_minmax(0,1fr)] lg:items-start">
        <aside className="overflow-hidden rounded-[2rem] border border-[#eceae4] bg-[#fffefb] shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
          <div className="relative h-[210px] sm:h-[250px] lg:h-[230px]">
            <img
              src={profileCard.cover}
              alt={`${profileCard.name} cover`}
              className="h-full w-full object-cover"
            />
          </div>

          <div className="bg-[#f3f2ef] px-8 pb-9 pt-7 sm:px-10">
            ...
            <p className="mt-2 text-[1.1rem] font-medium text-[#4b4e59]">
              {profileCard.handle}
            </p>
              ...
              {profileCard.contactDetails.map((item) => {
                ...
                return (
                  <div key={item.label} className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-[1rem] text-[#4b4e59] sm:text-[1.05rem]">
                        {item.label}
                      </p>
                      <p className="mt-1 text-[1.35rem] font-semibold leading-tight tracking-[-0.04em] text-[#161821] sm:text-[1.5rem]">
                        {item.value}
                      </p>
                    </div>
                  </div>
            ...
            <div className="mt-10 flex items-center justify-between gap-4 rounded-full bg-[#f7f7f4] px-5 py-4">
              <span className="truncate text-[1rem] text-[#161821] sm:text-[1.05rem]">
                {profileCard.copyUrl}
              </span>
