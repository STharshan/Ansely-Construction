...
export default function TestimonialGrid({ section }) {
  return (
    <section className="bg-[#f7f7f4] px-6 py-10 sm:px-10 sm:py-14 lg:px-32 lg:py-20">
      <div className="mx-auto max-w-[1480px]">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-7 text-[44px] font-semibold tracking-[-0.03em] text-[#161821] sm:text-[52px]">
              {section.title}
            </h2>
          </div>

          <button className="inline-flex items-center gap-2.5 self-start text-[15px] font-medium text-[#161821] transition hover:text-black lg:self-auto">
            {section.actionLabel}
            <ArrowRight size={18} />
          </button>
        </div>
        ...
        <div className="mt-10 grid gap-6 lg:max-w-[950px] lg:grid-cols-2">
          {section.items.map((item) => (
            <article key={item.title} className="group px-1 sm:px-2">
              ...
              <h3 className="mt-5 max-w-xl text-[20px] font-semibold leading-[1.14] tracking-[-0.03em] text-[#161821] sm:text-[24px]">
                {item.title}
              </h3>
              <div className="mt-7 flex items-center justify-between gap-4 text-[15px]">
