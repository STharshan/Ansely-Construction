...
function DetailLink({ text, href }) {
  return (
    <a
      href={href}
      className="inline-flex items-center gap-3 text-lg font-semibold text-[var(--color-ink-soft)] transition-colors duration-500 group-hover:text-white"
    >
      <span>{text}</span>
      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--color-brand)] text-white transition-transform duration-500 group-hover:rotate-45">
    ...
    <section className="relative overflow-hidden bg-[var(--color-panel)] px-4 py-20 pb-32">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <h2 className="mt-6 text-[2.5rem] font-semibold leading-[1.02] tracking-[-0.06em] text-[var(--color-ink)] sm:text-[3.4rem]">
        ...
        <div className="mt-20 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {builderCategories.map((item) => {
            ...
            return (
              <article
                key={item.title}
                className="group relative flex flex-col overflow-hidden rounded-[40px] bg-[var(--color-brand-mist)] p-8 transition-all duration-500 ease-in-out sm:p-10"
              >
                <div
                ...
                <div className="relative z-10 flex h-full flex-col">
                  <div className="flex h-[52px] w-[52px] items-center justify-center rounded-full bg-[var(--color-brand)] text-white transition-transform duration-500 group-hover:scale-110">
                  ...
                  <p className="mt-16 text-lg leading-[1.7] text-[var(--color-ink-muted)] transition-colors duration-500 group-hover:text-white/90">
                    {item.description}
                  </p>
